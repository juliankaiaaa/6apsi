...
The JavaScript you need before React. React is *just* JavaScript: components are
functions, props are objects, and lists are rendered with `.map()`. This unit
explains every fundamental the Module 1 activities lean on, and how each one
shows up later in React.

This is the **reading** that pairs with the practice:

- **Activity 1 (`m1a1`)** - print a message to the console and fill in your
  `student.json`.
- **Activity 2 (`m1a2`)** - implement small functions, one per concept below,
  until the tests pass.

The examples here are **not** the answers to the activity exercises; they show a
different use of the same idea, so you still get to work your own functions out.
Every section links to the MDN reference when you want more.

> **How to read this:** skim a section, try the matching exercise, come back when
> you get stuck. You don't need to memorize anything; understanding *what* each
> tool does is enough.
...
> (a cart sum). This is the most-used section in the whole module.
...
The async exercise in Activity 2 puts this together: `fetchUser` throws when the
id isn't found, and a `try / catch` around it logs the message instead of letting
the program crash.
